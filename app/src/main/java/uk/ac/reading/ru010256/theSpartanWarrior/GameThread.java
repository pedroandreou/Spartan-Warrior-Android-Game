...
import uk.ac.reading.ru010256.theSpartanWarrior.activities.highScore.HighScoreWriterActivity;
...
public abstract class GameThread extends Thread {
    ...
    public void run() {
        while (mRun) {
            canvasRun = null; //clear the canvas reference
            try {
                canvasRun = mSurfaceHolder.lockCanvas(null); //retrieve the current Canvas
                synchronized (monitor) {
    ...
    private void updatePhysics() { //show how much time has gone between frames
    ...
    abstract protected void onTouchUp();//have to override this in my subclass
    ...
    public boolean onTouch(MotionEvent e) {
        if (e.getAction() == MotionEvent.ACTION_DOWN) {
            ...
            if (mMode == STATE_LOSE){
                // As soon as the player loses, the player will be asked to set his credentials to store his high score
                Intent intent = new Intent(mGameView.getContext(), HighScoreWriterActivity.class);
                // Pass the high score of the user
                intent.putExtra("high_score", Long.toString(getScore()));
